...
from toad.utils import to_ndarray, bin_by_splits, save_json, read_json
from toad.utils.mixin import RulesMixin, BinsMixin
...
class ScoreCardnew(BaseEstimator, RulesMixin, BinsMixin):
    def __init__(self, pdo = 50, rate = 2, base_odds = 0.05, base_score = 600,
        card = None, combiner = {}, transer = None, lrmodel = None, **kwargs):
        """
        Args:
    ...
    def proba_to_score(self, prob):
        ...
        """
        return -self.factor * (np.log(prob) - np.log(1-prob)) + self.offset
    ...
    def score_to_proba(self, score):
        ...
        """
        return 1-(1 / (np.e ** ((-score + self.offset) / self.factor) + 1))
